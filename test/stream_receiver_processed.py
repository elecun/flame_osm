...
CONNECT_ADDRESS = "tcp://192.168.100.91:5203"
TOPIC = "image_stream_1_processed_monitor"
...
def main():
    ...
    print(f"Saving detected faces to: {OUTPUT_PATH}")
    ...
    try:
        while True:
            ...
            # Convert bytes (JPEG) to numpy array
            ...
            if image is not None:
                frame_count += 1
                h, w = image.shape[:2]
                print(f"[Receiver] Success: Received processed frame #{frame_count} | Size: {w}x{h} | Meta: {meta}")
                
                # Save the image
                cv2.imwrite(OUTPUT_PATH, image)
                print(f"[Receiver] Saved result image with bounding box to {OUTPUT_PATH}")
                # We only need to check one frame for test verification
                break
            else:
