...
{
    if (!_instance)
        _instance = new kvaser_can_interface();
    return _instance;
}
void Release()
{
...
bool kvaser_can_interface::onInit()
{
    ...
    try {
        ...
        for (int i = 0; i < _can_channels; i++) {
            ...
            logger::debug("[{}] CAN Channel {}: Device Name: {}\tDevice Channel: {}", getName(), i, device_name,
                          ch_on_card);
        }
        ...
        if (_can_handle < 0) {
            ...
            if (nom_status != canOK) {
                ...
                logger::error("[{}] Failed to set Nominal bitrate {} bps with sample point {}%: {}", getName(), bitrate,
                              nominal_sp * 100.0, err);
                return false;
            }
            ...
            if (fd_status != canOK) {
                ...
                logger::error("[{}] Failed to set CAN FD data bitrate {} bps with sample point {}%: {}", getName(),
                              fd_data_bitrate, data_sp * 100.0, err);
                return false;
            }
            ...
            logger::debug("[{}] CAN FD channel {} opened & set arbitration bitrate {} bps (SP: {}%), data bitrate {} "
                          "bps (SP: {}%)",
            ...
            if (stat == canOK) {
                ...
                if (id == 0x120) {
                    if (dlc >= 1) {
                        ...
                        logger::info("[{}] Received CMD_DMS_1000ms: DMS_Enable set to {}", getName(),
                                     (dms_enable_val ? "ENABLE" : "DISABLE"));
                    } else {
